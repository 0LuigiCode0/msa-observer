...
import (
	...
	mongoStore "x-msa-observer/store/mongo"
...
func (d *db) Close() {
	d._mongo.conn.(*mongo.Client).Disconnect(core_helper.Ctx)
...
func connMongo(v *helper.DbConfig) (conn *mongo.Client, err error) {
	...
	conn, err = mongo.Connect(core_helper.Ctx, opt)
	if err != nil {
		return conn, fmt.Errorf("db not connected: %v", err)
	}
	if err = conn.Ping(core_helper.Ctx, nil); err != nil {
